# `.template.config` (MultiSourceGeneratorRepository)

Fork of MultiLibraryRepository. Combo, init, TemplateAssets, and host JSON stay. `src/prj` is a Roslyn source-generator package: `netstandard2.0`, `analyzers/dotnet/cs`, `DevelopmentDependency`, direct GeneratorDriver tests, and an always-on Console DebugHost for Visual Studio `DebugRoslynComponent`. Tests, DebugHost, and optional benchmark share one DebugHost target framework from the wizard (select .NET 8, 9, or 10; not a library-style TFM matrix). The scaffold marks C# classes with `GenerateJsonSupportAttribute` and generates `TypeNameJson` helpers; it performs no file I/O or dynamic compilation. No UseWebSdk. No `ChoosingPackageBoundaries.md`.

Product surface: **`.NET multi-source-generator repository`**. `identity` is `CoreeTemplatesProjectMultiSourceGeneratorRepository`; CLI short name is `multisourcegeneratorrepo-coree`.
...
The template bootstraps a **repository layout** for one or more packable Roslyn source-generator packages. It does not `git init`. Same `--output` = combo repo; different `--output` = separate repos.
...
```powershell
dotnet new install "C:\dev\github.com\carsten-riedel\Coree.Template.Project\src\prj\Coree.Template.Project\Templates\MultiSourceGeneratorRepository" --force
```
...
Combo repo, three source-generator packages, root files only once:

```powershell
dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "Organization.Domain.SourceGenerator1" --output "C:\Users\Valgrind\source\repos\MultiSourceGeneratorRepository-multisolution-optin" --InitAllRepoItems
dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "Organization.Domain.SourceGenerator2" --output "C:\Users\Valgrind\source\repos\MultiSourceGeneratorRepository-multisolution-optin"
dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "Organization.Domain.SourceGenerator3" --output "C:\Users\Valgrind\source\repos\MultiSourceGeneratorRepository-multisolution-optin"
```
...
```powershell
dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "Organization.Domain.SourceGenerator1" --output $out --InitAllRepoItems
```
...
```powershell
# combo gut
dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "...SourceGenerator1" --output $out --InitAllRepoItems
dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "...SourceGenerator2" --output $out
...
# dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "...SourceGenerator1" --output $out --InitAllRepoItems
# dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "...SourceGenerator2" --output $out --InitAllRepoItems
```
...
dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "...SourceGenerator1" --output $out --InitAllRepoItems --NerdbankGitVersioning Repo
dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "...SourceGenerator2" --output $out --NerdbankGitVersioning Repo
...
# dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "...SourceGenerator1" --output $out --InitAllRepoItems --NerdbankGitVersioning Repo
# dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "...SourceGenerator2" --output $out --InitAllRepoItems --NerdbankGitVersioning Repo
```
...
```powershell
dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "Organization.Domain.LibA" --output $outA --InitAllRepoItems
dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "Organization.Domain.LibB" --output $outB --InitAllRepoItems
```

Each `--output` is its own first create.

After the first call in the default combo the repo root has `README.md`, `TEMPLATE-AI-RELEASE-CHECKPOINT.md`, `.gitattributes`, `.gitignore`, and `LICENSE`. Each package gets `Properties/version.json`. Calls 2 and 3 add `src/prj` / `src/sln` trees only. Passing `--InitAllRepoItems` or `--InitRepoItems Readme` again into the same folder is Exit 73 (collision); `--force` would overwrite. `--InitRepoItems SrcGlobalJson` is a separate first-create opt-in (`src/global.json`, default off).
...
```powershell
dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "Organization.Domain.SourceGenerator1" --output "<repo>" --InitRepoItems AIReleaseCheckpoint
```
...
A combo repository is two or more `dotnet new` calls into the **same** `--output`. The template engine has **one** CLI default for every call. If `InitRepoItems` defaulted to the five root files, the second package would hit Exit 73 (collision) unless the caller passed `None` or `--force`. Visual Studio’s New Project dialog is a **first create** into an empty folder; it can check the five boxes by default. CLI later-packages omit `--InitAllRepoItems` and `--InitRepoItems`. `NerdbankGitVersioning` defaults to **`Project`** on both hosts (per-package `Properties/version.json`; later creates do not collide). `--NerdbankGitVersioning Repo` does not write the root `version.json` by itself (`WriteRepoVersionJson` does), so a later package can pass `--NerdbankGitVersioning Repo` again without Exit 73.
...
`SrcGlobalJson` is `src/global.json`. Opt-in, **default off**, not part of `--InitAllRepoItems` or the Visual Studio first-create checks. Exclude unless this choice is selected (`None` still wins). Seed lives at that output path (not under `TemplateAssets/`). `SdkPinVersion` follows the selected **DebugHost** TFM (`8.0.0` / `9.0.0` / `10.0.0`), not netstandard2.0; `rollForward` is `latestFeature`. Later package with this box selected is Exit 73. Commands started at the repository root do not see this file.
...
Source-generator-only single choice, default **`Strict`**. UI label **Code style rules for source-generator project** (`is_global` is the analyzer-config technical term). CLI long name **`--ProjectEditorGlobalConfig`**. Stamps `src/prj/{Name}/.project.editor.globalconfig` and wires `GlobalAnalyzerConfigFiles`, `EnforceCodeStyleInBuild`, and `OptimizeImplicitlyTriggeredBuild=false`. `Off` writes nothing. Tests and benchmark do not get the file. Keep the symbol and generated disk name. Do not use a bare "project" label: **C# project options** already applies to source generator, tests, DebugHost, and benchmark.
...
`Strict` needs the product C# defaults (`GenerateDocumentationFile`, `Nullable`, `DisableImplicitUsings`) or those errors fire on every build for the wrong reason. `JsonSupportGenerator` already has XML documentation and no deliberately unused usings, so the first Strict build passes without a scaffold-only conditional.
...
This PropertyGroup/`GlobalAnalyzerConfigFiles` lives in the source-generator csproj. The project is a stock SDK project (`Project Sdk="Microsoft.NET.Sdk"`), so Sdk.targets loads last by default. The `None` Link is Solution Explorer only and can sit in the same `#if` block.

## `AnalysisMode`

Source-generator-only single choice, default **`Recommended`**. UI label **Code analysis mode**. CLI long name **`--AnalysisMode`**. Omit the switch → `Recommended`. Writes `<AnalysisMode>` on the source-generator project only (placeholder `__AnalysisMode__`). Tests and benchmark do not get the property. Combo-safe.

This is the SDK **CA** rule set, not code style and not Public API analyzers. Do not merge it into `ProjectEditorGlobalConfig`. Do not add `None` or SDK `Default`: a packable source generator keeps analyzers on; `Minimum` / `Recommended` / `All` are the three product values. Do not add `AnalysisLevel` on the same switch (that pins a SDK rule version). Do not add `EnableNETAnalyzers` `true` noise.

Warnings only unless the project later sets `TreatWarningsAsErrors`. `All` can be noisy on generator infrastructure.

The PropertyGroup lives in the source-generator csproj (stock SDK project).
...
SDK restore already runs NuGetAudit (NU1901–NU1904 warnings). This switch only appends `<WarningsAsErrors>$(WarningsAsErrors);NU1903;NU1904</WarningsAsErrors>` on the packable source-generator. Low (`NU1901`) and moderate (`NU1902`) stay warnings. Do not set `NuGetAudit` / `NuGetAuditMode` / `TreatWarningsAsErrors` here (`NuGetAudit` is already on). Do not promote the test-project `WriteNugetReport` `dotnet list package` files: that command’s exit code is not a findings gate.
...
```powershell
dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "...SourceGenerator1" --output $out --InitAllRepoItems
dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "...SourceGenerator2" --output $out --NuGetAuditHighCriticalAsErrors false
```
...
```powershell
dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "...SourceGenerator1" --output $out --InitAllRepoItems --PublicApiAnalyzers
dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "...SourceGenerator2" --output $out --PublicApiAnalyzers
```
...
```powershell
# combo gut
dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "...SourceGenerator1" --output $out --InitAllRepoItems --DocumentationTemplate Package Repository
dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "...SourceGenerator2" --output $out --DocumentationTemplate Package

# combo error (Exit 73) — Call 2 stamps docs/DocShell.html again
# dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "...SourceGenerator1" --output $out --InitAllRepoItems --DocumentationTemplate Repository
# dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "...SourceGenerator2" --output $out --DocumentationTemplate Repository
```
...
The source generator is packable. `IsPublishable` is `false` (NuGet pack is the distribution path). Single `TargetFramework` `netstandard2.0`, stock `<Project Sdk="Microsoft.NET.Sdk">`. There is no `PublishDefaultFramework` / `ImportSdkTargets` pair. `Properties/Build/PackAsSourceGenerator.targets` puts the assembly and XML documentation under `analyzers/dotnet/cs`; no consumer `build/` props are required by the scaffold. `Properties/NugetMetadata/` is nupkg assets (readme, icon, notes, optional `docs/`). Both folders are on disk under `Properties/` so Explorer and Solution Explorer match; they are not source. `.project.editor.globalconfig`, `.config/dotnet-tools.json`, and `Directory.Build.*` stay next to the csproj. `Properties/AssemblyInfo.cs` grants `InternalsVisibleTo` the test assembly (`__SourceName__.Tests` via `sourceName`). The test project’s root `AssemblyInfo.cs` is only MSTest `Parallelize`. DebugHost is always generated: a Console consumer and compile target for Visual Studio `DebugRoslynComponent` (`Properties/launchSettings.json` on the source generator, `OutputItemType` Analyzer / `ReferenceOutputAssembly` false on the host). Tests, DebugHost, and the optional BenchmarkDotNet executable explicitly set `IsPackable` and `IsPublishable` to `false`. Tests, DebugHost, and benchmark share one runnable TFM from the wizard (default `net10.0`). The source generator itself is always `netstandard2.0` and does not multi-target. Versioning default is Nerdbank **Project** (`Properties/version.json`). Tests, DebugHost, and benchmark are not packable. **`--NerdbankGitVersioning`** `Off` is VersionPrefix; `Repo` is the shared root file.

**Test project layout.** Mini-scopes in this order: runnable TFM, language/debug (from `CSharpProjectOptions`), packaging, test configuration (MSTest logger), Coverlet `#if` block, ReportGenerator `#if` block, NugetReport + `WriteNugetReport`, `.gitignore` hide, `ProjectReference`, then **External dependencies** `PackageReference`s last. Coverage is **`--TestCoverage`**: `Coverlet` (default), `CoverletAndReport`, `None`. Do not restore independent Coverlet/ReportGenerator bools: ReportGenerator consumes `@(CoverletReport)`. Coverlet is `coverlet.msbuild` + `CollectCoverage=true`; that **does** run on `dotnet test` (VSTest path, SDK 10), including Linux/WSL (`dotnet` ships MSBuild). The Coverlet `#if` also writes `Include` `[__SourceName__]*` (sourceName → the source-generator assembly only) and `Threshold` `100` / `line,branch,method` / `total`. Those are generate-time properties, not wizard fields: Visual Studio cannot show extra inputs only when Coverlet is selected. `--TestCoverage None` omits the whole PropertyGroup. `JsonSupportGeneratorTests` runs the generator through `CSharpGeneratorDriver`, compiles the output, exercises the generated JSON round trip, and covers unsupported generic/nested targets so a first `dotnet test` passes the 100% gate. Report/logger/NugetReport paths use `$([MSBuild]::NormalizeDirectory(...))` so Linux does not create a folder named `ReportGeneratorOutput\net10.0`.

**Why there is no `PublishDefaultFramework`.** The source generator is one TFM (`netstandard2.0`). Stock `Project Sdk="Microsoft.NET.Sdk"` is enough. Tests do not `SetTargetFramework` on the source-generator `ProjectReference`. `SourceControlState.targets` is a `BeforeTargets` hook on `GenerateAssemblyInfo` (SDK 8+ Source Link). `InitializePublicApi.targets` is imported only when `PublicApiAnalyzers` is on.

Source generator, tests, DebugHost, and optional benchmark each remove `.gitignore` from their `None` items so it stays on disk without appearing as a project item. The test ignore also covers generated `NugetReport/` output.

The packable source-generator sets `EnablePackageValidation` to `false` (no `lib/` TFMs). Tests, DebugHost, and benchmark are not packable.

With `PlaceSolution` `SlnFolder` (default), each `src/sln/{Name}/` gets its own `.gitignore` for `.vs/` next to that package’s `.slnx` and handbook `Readme.md`. `RepoRoot` still writes `src/sln/{Name}/Readme.md` as notes (no nested `.gitignore`); delete that folder if you do not need it. `BesideCsproj` writes the `.slnx` and handbook next to the packable source-generator csproj and does **not** create `src/sln/{Name}/` (`src/prj/{Name}/.gitignore` already ignores `.vs/`). Repository-root `.gitignore` is `InitRepoItems` `GitIgnore` / `--InitAllRepoItems` (first create only). Later packages omit Init and do not overwrite it. If root ignore is off, the `RepoRoot` variant and Visual Studio’s extra root `.vs/` stay the owner’s problem.

## `DirectoryMsBuildFiles`

Source generator + this package's `.slnx` only. Bool, default **false**. UI label **Empty Directory.Build and Directory.Solution files**. CLI long name **`--DirectoryMsBuildFiles`**. Omit the switch → nothing. Combo-safe on `SlnFolder` and `BesideCsproj` (paths include the package name). `RepoRoot` stacks `Directory.Solution.*` at the repository root like stacking `.slnx` files. Not an Init* item and not `Directory.Packages.props` (CPM would not cover sibling tests). Tests, DebugHost, and benchmark are not in this switch.

Seeds live under `TemplateAssets/DirectoryMsBuild/`. Extra sources copy `Directory.Build.props` / `.targets` next to the source-generator csproj, and `Directory.Solution.props` / `.targets` next to this package's `.slnx` (`src/sln/{Name}/` for `SlnFolder`, `src/prj/{Name}/` for `BesideCsproj`, repo root for `RepoRoot` — later package then collides, same as stacking `.slnx` files). DocShell extra sources must `exclude` `Versioning/**` and `DirectoryMsBuild/**`.

The files are almost empty `<Project>` stubs with comments. MSBuild auto-imports them from those directories. Do not move `PackAsSourceGenerator` or generator configuration into them. The template does not `#if` properties into these files vs the csproj (possible, ugly). The source-generator csproj `None Include`s the two `Directory.Build.*` files with `Link` under `Properties\` (Solution Explorer only). Do **not** move the files into `Properties/` on disk: auto-import follows the directory of the file, same as `.project.editor.globalconfig`. `Directory.Solution.*` stay beside the `.slnx`. When `PlaceSolution` is `BesideCsproj`, the source-generator csproj also `Link`s those two solution files (same folder as the csproj) so they do not look like stray project items.

```powershell
dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "...SourceGenerator1" --output $out --InitAllRepoItems --DirectoryMsBuildFiles
dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "...SourceGenerator2" --output $out --DirectoryMsBuildFiles
```

## `DotNetToolManifest`

Analyzer project only. Bool, default **true**. UI label **Empty local dotnet-tools.json**. CLI long name **`--DotNetToolManifest`**. Omit the switch → empty `src/prj/{Name}/.config/dotnet-tools.json`. `--DotNetToolManifest false` skips it. Combo-safe: path includes the package name. Not Init. `tools` is `{}`; no `dotnet tool restore` on build. Tests, DebugHost, and benchmark are not in this switch. `isRoot` is true so a later parent manifest does not merge in. `dotnet tool install --local` from the analyzer folder fills the file. With `SlnFolder`, the handbook CWD (`src/sln/{Name}/`) does not see this manifest. With `BesideCsproj`, the handbook CWD is the analyzer folder and does.

The source-generator csproj `None Include`s the file with `Link` under `Properties\` (Solution Explorer only). Disk path stays `.config/`.

```powershell
dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "...SourceGenerator1" --output $out --InitAllRepoItems
dotnet new multisourcegeneratorrepo-coree --Author "abcd" --name "...SourceGenerator2" --output $out --DotNetToolManifest false
```

## Other symbols worth not breaking

- **`ProjectLicense` / `RepoLicense` / `InitAllRepoItems` / `GitIgnore` / `SrcGlobalJson`**: see section above. Default MIT. SPDX expression for standards; `PackageLicenseFile` only for `Custom`. Root `LICENSE` is the `RepoLicense` item or the CLI all-set. Root `.gitignore` is `GitIgnore` in that same first-create set. `src/global.json` is `SrcGlobalJson`: opt-in, default off, not in `--InitAllRepoItems`; pin follows DebugHost TFM, not netstandard2.0.
...
- **`ProjectEditorGlobalConfig`**: see section above. Default `Strict`. `Default` is suggestions-only. Seeds in `TemplateAssets/CodeStyle/`; generated disk name stays `.project.editor.globalconfig`. Csproj wire-up on the source generator only.
- **`AnalysisMode`**: see section above. Default `Recommended`. Source generator only. CA warnings, not build errors.
- **`NuGetAuditHighCriticalAsErrors`**: see section above. Default `true`. Source generator only. NU1903/NU1904 as restore errors. Off for EOL/backport graphs.
- **`PublicApiAnalyzers`**: see section above. Default `false`. Source generator only. Baseline files are first-build, not generate-time.
...
- **`EnablePackageValidation`**: not a symbol. Always `false` on the source generator (no `lib/` TFMs).
...
- **`DirectoryMsBuildFiles`**: see section above. Default `false`. Empty `Directory.Build.*` beside the source generator and `Directory.Solution.*` beside this `.slnx`. No `Directory.Packages.props`.
