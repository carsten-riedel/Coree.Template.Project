...
After creation in Visual Studio, close Visual Studio and run `powershell -NoProfile -ExecutionPolicy Bypass -File .\src\wrk\ClassLibrary\Complete-Repository.ps1` from the repository root. The script preserves the redundant outer solution as a backup, completes the layout, and opens the solution in VS 2026. For a layout-only run, add `-NoOpen`.

For the commands below, open a terminal in the repository's `src/` directory (the solution root). The CLI finds the solution; you do not pass a `.slnx` or `.csproj` path.
