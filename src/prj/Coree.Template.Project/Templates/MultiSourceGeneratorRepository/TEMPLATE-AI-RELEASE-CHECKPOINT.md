...
2. Every packable library `src/prj/*/Properties/NugetMetadata/Readme.md` describes that library's public surface. Empty or placeholder NuGet readmes are not done.
...
6. Public surface has no leftover template samples (`JsonSupportGenerator`, `GenerateJsonSupportAttribute`, DebugHost `Customer`, tests that do not assert the product, copyright lines that still lie).
