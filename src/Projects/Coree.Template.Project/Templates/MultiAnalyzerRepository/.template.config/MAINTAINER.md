...
Product surface: **`.NET analyzer package repository`**. `identity` is `CoreeTemplatesProjectMultiAnalyzerRepository`; CLI short name is `analyzerrepo-coree`.
...
**Visual Studio:** first create uses the `ide.host.json` default (same five root files as `--InitAllRepoItems`). A second package in the IDE cannot omit the group: choose **None**. Folgelibraries in the same folder are otherwise the CLI path above. Why the two hosts differ is in **CLI ↔ Visual Studio** below.
...
- **`PlaceSolution` `RepoRoot`:** CLI renames to `{Name}.slnx` at repo root. Visual Studio keeps `{Name}.generated.slnx` so it does not overwrite the `{Name}.slnx` the IDE always writes. `SlnFolder` and `BesideLibrary` rename on both hosts (paths are not the VS root file). Post-actions that open the sln readme and tell you to close/reopen are `HostIdentifier == "vs"` only.
...
With `PlaceSolution` `SlnFolder` (default), each `src/sln/{Name}/` gets its own `.gitignore` for `.vs/` next to that package's `.slnx`. `src/sln/{Name}/Readme.md` is always written: notes and optional cross-project files; delete the folder only when the `.slnx` is not there and you do not need it. `BesideLibrary` puts the `.slnx` next to the packable analyzer csproj (`src/prj/{Name}/.gitignore` already ignores `.vs/`). Repository-root `.gitignore` is `InitRepoItems` `GitIgnore` / `--InitAllRepoItems` (first create only). Later packages omit Init and do not overwrite it. If root ignore is off, the `RepoRoot` variant and Visual Studio's extra root `.vs/` stay the owner's problem.
...
Seeds live under `TemplateAssets/DirectoryMsBuild/`. Extra sources copy `Directory.Build.props` / `.targets` next to the analyzer csproj, and `Directory.Solution.props` / `.targets` next to this package's `.slnx` (`src/sln/{Name}/` for `SlnFolder`, `src/prj/{Name}/` for `BesideLibrary`, repo root for `RepoRoot` — later package then collides, same as stacking `.slnx` files). `TemplateAssets/` extra sources for `version.json` and DocShell must `exclude` `DirectoryMsBuild/**`.
...
- **`PlaceSolution`**: single choice, default `SlnFolder` → `src/sln/ClassLibrary/ClassLibrary.slnx` (one `.slnx` per folder so `dotnet` / CI do not see sibling solutions). `src/sln/{Name}/` is always created (notes / cross-project files). `RepoRoot` on CLI renames to a root `.slnx`; `RepoRoot` in Visual Studio keeps `*.generated.slnx` so it does not overwrite VS’s conventional root `.slnx`. `RepoRoot` also stacks every package’s `.slnx` in one directory. `BesideLibrary` writes `src/prj/{Name}/{Name}.slnx` next to the packable analyzer csproj (not tests, DebugHost, or benchmark).
