...
- **`PlaceSolution`**: single choice, default `SlnFolder` → `src/sln/ClassLibrary/ClassLibrary.slnx` (one `.slnx` per folder so `dotnet` / CI do not see sibling solutions). `src/sln/{Name}/` is always created (notes / cross-project files). `RepoRoot` on CLI renames to a root `.slnx`; `RepoRoot` in Visual Studio keeps `*.generated.slnx` so it does not overwrite VS’s conventional root `.slnx`. `RepoRoot` also stacks every library’s `.slnx` in one directory. `BesideLibrary` writes `src/prj/{Name}/{Name}.slnx` next to the packable csproj.
