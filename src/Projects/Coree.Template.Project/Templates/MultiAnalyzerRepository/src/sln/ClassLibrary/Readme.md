...
3. Set a breakpoint in `SampleAnalyzer`.
4. Press F5. Visual Studio compiles `ClassLibrary.DebugHost` and attaches to that compilation.

`ClassLibrary.DebugHost` is only the compile target. Types there trigger ANL001 (`lowercase` vs `UPPER`). F5 / `dotnet run` on the console only runs `Main`; it does not attach to the analyzer.
...
Creates one `.nupkg` in `src/prj/ClassLibrary/bin/Pack/` with the analyzer under `analyzers/dotnet/cs` (not `lib/`). Test, DebugHost, and optional benchmark projects are not packed.
