...
5. NuGet license metadata matches what you ship: SPDX `PackageLicenseExpression` for MIT / BSD 3-Clause / Apache 2.0, or the packed `NugetAssets/License.txt` when the license is custom (copyright-only until you add grant terms). Replace the icon only if the default asset must not ship.
