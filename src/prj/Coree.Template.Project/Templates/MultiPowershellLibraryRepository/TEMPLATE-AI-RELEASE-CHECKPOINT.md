...
4. `Properties/NugetAssets/__SourceName__.psd1` has accurate description, tags, project URI, license URI, prerelease metadata, exports, compatible editions, and minimum PowerShell version.
...
6. `Properties/NugetAssets/License.txt`, the optional repository-root `LICENSE`, and published license metadata agree. Replace the scaffold icon if it should not ship.
7. Remove or replace `Get-SampleValue`, its formatter, tests, benchmark, and debug invocation with real product behavior.
8. `dotnet pack -c Release` on the module project stages a clean Release module and writes a PowerShell Gallery-compatible `.nupkg` under `bin/Pack`; it must not publish a normal class-library `lib/` NuGet layout.
